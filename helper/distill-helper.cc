...
NS_LOG_COMPONENT_DEFINE ("DistillHelper");

DistillHelper::DistillHelper (Ptr<QuantumPhyEntity> qphyent, bool checker, Ptr<QuantumChannel> conn)
{
  m_factory.SetTypeId (DistillApp::GetTypeId ());
...
void
DistillHelper::SetAttribute (std::string name, const AttributeValue &value)
{
  m_factory.Set (name, value);
}

ApplicationContainer
DistillHelper::Install (Ptr<Node> node) const
{
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer
DistillHelper::Install (std::string nodeName) const
{
...
ApplicationContainer
DistillHelper::Install (NodeContainer c) const
{
...
Ptr<Application>
DistillHelper::InstallPriv (Ptr<Node> node) const
{
  Ptr<Application> app = m_factory.Create<DistillApp> ();
